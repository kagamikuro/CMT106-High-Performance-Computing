...
int main (int argc, char *argv[])
{
   ...
   int icheck, i, j, alpha, beta, nlocal, rem, nbeg, nend, istart, iend;
...
/* Set up 1D topology */
   ...
   nlocal  = (npts-1)/nprocs + 1; // ceil(npts/nprocs)
   rem = npts%nprocs;
   if (rem!=0) nlocal = (rank<rem ? nlocal : nlocal-1);
   nbeg = (rank <rem ? nlocal*rank : nlocal*rank+rem);
   nend = nbeg + nlocal - 1;
...
/* Do the updates */
   ...
   for(j=0;j<NSTEPS;j++){
         ...
         if(j%OUTFREQ == 0) {
                count_out++;
		output_solution(psi, buf, new_comm, rank, mypos, nprocs, npts, nlocal, count_out);
         }
   }
/*
   if(rank==0){
...
int output_solution(double *psi, double *buf, MPI_Comm new_comm, int rank, int mypos, int nprocs, int npts, int nlocal, int count_out)
{
    int alpha, beta, nbeg, i, j, proc, count, tag=99;
    MPI_Status status;
    FILE *fp;
    const char base[] = "OUT/";
    ...
    if(rank==0){
         ...
         fp = fopen(filename,"w");
         for(j=0;j<nprocs;j++){
             nbeg = (j<beta) ? (alpha+1)*j : alpha*j+beta;
             if (mypos!=j){
                 MPI_Cart_rank (new_comm, &j, &proc);
                 MPI_Recv (&buf[1], nlocal, MPI_DOUBLE, proc, tag, new_comm, &status);
                 MPI_Get_count(&status, MPI_DOUBLE, &count);
                 for(i=1;i<=count;i++)
//                     printf("%9.6f%c",old_psi[i],(i%8==0) ? '\n' : ' ');
